...
var app = (function () {
    ...
    function run(input, vars = {}) {
        ...
        EXP.setPattern(lib.alt(lib.lrec_sc(FACTOR, lib.seq(lib.tok(K.Op2), FACTOR), expOp), lib.apply(lib.seq(lib.tok(K.Name), lib.tok(K.Assign), EXP), expAssign)));
        return lib.expectSingleResult(lib.expectEOF(EXP.parse(lexer.parse(input))));
    }
    /*
    while i < 10
        j = 0
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(main, file, 16, 0, 496);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	addEventListener("DOMContentLoaded", () => {
    		...
    		function exec() {
    			...
    			$$invalidate(0, board[i][j] = color, board);
    		}
